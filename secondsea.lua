...
queue_on_teleport([[
    loadstring(game:HttpGet("https://raw.githubusercontent.com/0xNyaku/ravenhub/main/kayzhub.lua"))()
]])
...
local Window = SpecterHub:CreateWindow("<font color=\"#7f4ff7\">KayZ Hub - By Nyaku - Second Sea</font>", txt, true)
...
if (executor == "Synapse X" or executor == "Krnl") and secure_call then
    ...
    SpecterHub:AddNoti("Warning", "Your Exploit Is Not Supported, Our Script's might not working perfectly (Supported Exploit: Synapse X)", 1)
end
...
function GetAllMobInShip(ship) 
    ...
    for k,v in pairs(game:GetService("Workspace").NPCs:GetChildren()) do 
        if v:FindFirstChild("assignedShip") and v:FindFirstChild("Humanoid") and v.Humanoid.Healfth>0 then 
...
Section2:CreateToggle("Auto Observation V2", {Description = "Do the Minigame for you"}, function(state)
    if state then
